    ...
    fun getSlots(date: String) {
        API.get().checkVaccinesByPincode(pincode, date).enqueue(object : Callback<ApiResponseModel<ArrayList<APIPincodeResult>>> {
            ...
            override fun onFailure(call: Call<ApiResponseModel<ArrayList<APIPincodeResult>>>, t: Throwable) {
                Log.d("BRANCH_APP",t.message.toString())
                loadingDialog.dismissDialog()
         ...
         for (i in 0..apiSessions.lastIndex){
             ...
             val db = DatabaseHandler(this)
                 if (dose.toInt()==1 && apiSessions[i].available_capacity_dose1>0){
                     ...
                     var data : CentersData = CentersData(apiSessions[i].center_id,apiSessions[i].name, apiSessions[i].address, apiSessions[i].pincode,apiSessions[i].date, apiSessions[i].from, apiSessions[i].to, apiSessions[i].lat, apiSessions[i].long, paid, apiSessions[i].vaccine, "Dose 1",
                         apiSessions[i].fee.toInt(),apiSessions[i].min_age_limit,apiSessions[i].max_age_limit, apiSessions[i].available_capacity_dose1)
                     centersList.add(data)
                 }
                 else if (dose.toInt()==2 && apiSessions[i].available_capacity_dose2>0){
                     var data : CentersData = CentersData(apiSessions[i].center_id,apiSessions[i].name, apiSessions[i].address, apiSessions[i].pincode,apiSessions[i].date, apiSessions[i].from, apiSessions[i].to, apiSessions[i].lat, apiSessions[i].long, paid, apiSessions[i].vaccine, "Dose 2",
                         apiSessions[i].fee.toInt(),apiSessions[i].min_age_limit,apiSessions[i].max_age_limit, apiSessions[i].available_capacity_dose2)
                     centersList.add(data)
                 }
                 else{
                     var data : CentersData = CentersData(apiSessions[i].center_id,apiSessions[i].name, apiSessions[i].address, apiSessions[i].pincode,apiSessions[i].date, apiSessions[i].from, apiSessions[i].to, apiSessions[i].lat, apiSessions[i].long, paid, apiSessions[i].vaccine, "", apiSessions[i].fee.toInt(),apiSessions[i].min_age_limit,apiSessions[i].max_age_limit, apiSessions[i].available_capacity)
                     centersList.add(data)
                 }
         }
        loadingDialog.dismissDialog()
